...
export async function callAnthropic(config, request) {
  ...
  // v4.2: Apply prompt caching for Anthropic (saves ~90% on cached tokens).
  let cachedSystem = system;
  let cachedMessages = messages;
  try {
    const { addAnthropicCacheControl } = await import("./caching.js");
    const cached = addAnthropicCacheControl(messages, system);
    cachedSystem = cached.system;
