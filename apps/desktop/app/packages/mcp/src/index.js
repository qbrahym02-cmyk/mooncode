...
export class McpRegistry extends EventEmitter {
  ...
  async addServer(id, config) {
    ...
    // v4.2: Support both stdio (command) and HTTP (url) servers.
    if (config.url) {
      servers[id] = { url: config.url, headers: config.headers || {}, transport: "http" };
    } else {
  ...
  async connect(id) {
    ...
    // v4.2: HTTP transport — use connectMcpHTTP instead of stdio spawn.
    if (config.transport === "http" || config.url) {
      try {
        const { connectMcpHTTP } = await import("./oauth.js").catch(() => ({})).then(() => ({})).catch(() => ({}));
        // Try importing connectMcpHTTP from the main module
        const httpInfo = await import("./index.js").then(() => null).catch(() => null);
        // Direct call to connectMcpHTTP (exported from this same file)
        const info = await connectMcpHTTP(config.url, { headers: config.headers || {}, id });
        // Create a lightweight HTTP client wrapper
        const httpClient = {
          id,
          transport: "http",
          httpUrl: config.url,
          ...
          async readResource(uri) {
            const res = await fetch(`${this.httpUrl}`, {
              method: "POST",
              headers: { "content-type": "application/json", ...this.httpHeaders },
              body: JSON.stringify({ jsonrpc: "2.0", id: 4, method: "resources/read", params: { uri } }),
            });
            const data = await res.json();
            return data.result;
          },
        ...
        this.clients.set(id, httpClient);
        return httpClient;
      } catch (error) {
        this.emit("error", { id, error: new Error(`HTTP MCP connect failed: ${error.message}`) });
        throw error;
      }
    }
